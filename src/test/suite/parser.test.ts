...
function mockDocWithName(text: string, fileName: string): vscode.TextDocument {
...
suite('parseStructs — file exclusions', () => {
  const src = 'type Foo struct {\n\tID int `json:"id"`\n}'

  test('_test.go suffix → empty result', () => {
    assert.deepStrictEqual(
      parseStructs(mockDocWithName(src, 'foo_test.go')),
      [],
    )
  })

  test('_generated.go suffix → empty result', () => {
    assert.deepStrictEqual(
      parseStructs(mockDocWithName(src, 'models_generated.go')),
      [],
    )
  })

  test('_gen.go suffix → empty result', () => {
    assert.deepStrictEqual(
      parseStructs(mockDocWithName(src, 'schema_gen.go')),
      [],
    )
  })

  test('.pb.go suffix → empty result', () => {
    assert.deepStrictEqual(
      parseStructs(mockDocWithName(src, 'service.pb.go')),
      [],
    )
  })

  test('.pb.gw.go suffix → empty result', () => {
    assert.deepStrictEqual(
      parseStructs(mockDocWithName(src, 'service.pb.gw.go')),
      [],
    )
  })

  test('Code generated comment in first line → empty result', () => {
    const generated = '// Code generated by protoc-gen-go. DO NOT EDIT.\n' + src
    assert.deepStrictEqual(
      parseStructs(mockDocWithName(generated, 'service.go')),
      [],
    )
  })

  test('Code generated comment within first 10 lines → empty result', () => {
    const prefix = Array(5).fill('// some comment').join('\n') + '\n'
    const generated =
      prefix + '// Code generated by stringer. DO NOT EDIT.\n' + src
    assert.deepStrictEqual(
      parseStructs(mockDocWithName(generated, 'enum_string.go')),
      [],
    )
  })

  test('regular .go file is NOT skipped', () => {
    const structs = parseStructs(mockDocWithName(src, 'models.go'))
    assert.strictEqual(structs.length, 1)
  })
})

suite('parseStructsWithFields — noReorder annotation', () => {
  test('// go-struct-tags:no-reorder on declaration line sets noReorder', () => {
    const src = [
      'type Foo struct { // go-struct-tags:no-reorder',
      '\tB byte',
      '\tX int64',
      '}',
    ].join('\n')
    const structs = parseStructsWithFields(mockDoc(src))
    assert.strictEqual(structs.length, 1)
    assert.strictEqual(structs[0].noReorder, true)
  })

  test('// betteralign:ignore on declaration line sets noReorder', () => {
    const src = [
      'type Bar struct { // betteralign:ignore',
      '\tB byte',
      '\tX int64',
      '}',
    ].join('\n')
  ...
  test('// go-struct-tags:no-reorder inside body sets noReorder', () => {
    const src = [
      'type Baz struct {',
      '\tB byte // go-struct-tags:no-reorder',
      '\tX int64',
      '}',
    ].join('\n')
    const structs = parseStructsWithFields(mockDoc(src))
  ...
  test('// betteralign:ignore inside body sets noReorder', () => {
    const src = [
      'type Qux struct {',
      '\tB byte // betteralign:ignore',
      '\tX int64',
      '}',
    ].join('\n')
    const structs = parseStructsWithFields(mockDoc(src))
  ...
  test('struct without annotation has noReorder false', () => {
    const src = ['type Clean struct {', '\tB byte', '\tX int64', '}'].join('\n')
    const structs = parseStructsWithFields(mockDoc(src))
    assert.strictEqual(structs[0].noReorder, false)
  })

  test('annotation on one struct does not affect sibling struct', () => {
    const src = [
      'type A struct { // betteralign:ignore',
      '\tB byte',
      '}',
      'type B struct {',
      '\tX int64',
    ...
    const structs = parseStructsWithFields(mockDoc(src))
    assert.strictEqual(structs[0].noReorder, true)
    assert.strictEqual(structs[1].noReorder, false)
  })

  test('file exclusion also applies to parseStructsWithFields', () => {
    const src = 'type Foo struct {\n\tID int\n}'
    assert.deepStrictEqual(
      parseStructsWithFields(mockDocWithName(src, 'foo_test.go')),
      [],
    )
  })

  test('inner struct fields are NOT included in outer allFields', () => {
    const src = [
      'type Outer struct {',
      '\tExtraField string',
      '\tUserProfile struct {',
    ...
    const structs = parseStructsWithFields(mockDoc(src))
    assert.strictEqual(structs.length, 1)
    const names = structs[0].allFields.map((f) => f.name)
    assert.ok(names.includes('ExtraField'), 'ExtraField must be in allFields')
    assert.ok(
      !names.includes('ID'),
      'ID (inner struct field) must NOT be in allFields',
    )
    assert.ok(
      !names.includes('FirstName'),
      'FirstName (inner) must NOT be in allFields',
    )
  })

  test('single field after fixing inner struct depth — no false reorder', () => {
    // With only ExtraField in allFields, optimalOrder sees 1 field → reordered=false
    const src = [
      'type Nested struct {',
      '\tAddress',
    ...
    const structs = parseStructsWithFields(mockDoc(src))
    assert.strictEqual(structs.length, 1)
    // Address is embedded (no type), UserProfile body is skipped — only ExtraField
    const names = structs[0].allFields.map((f) => f.name)
    assert.deepStrictEqual(names, ['ExtraField'])
  })
})
